...
class SearchArgs {
  ...
  bool operator ==(Object other) {
    ...
    return query == other.query &&
        ...
        const ListEquality().equals(sources, other.sources);
  }
}
...
class SearchNotifier extends AsyncNotifier<PaginatedResult<UnifiedMedia>?> {
  ...
  Future<PaginatedResult<UnifiedMedia>> _fetchCategoryPage(
    ...
    if (prefs.mode == MetadataMode.tracker) {
      ...
      final List<SourceInfo> targetSources;
      if (arg.sources.isNotEmpty) {
        targetSources = allSources
            .where((s) => arg.sources.contains(s.id))
            .toList();
      } else if (arg.source != null) {
        targetSources = allSources.where((s) => s.id == arg.source).toList();
      } else {
        final active = allSources
            .where((s) => prefs.activeSources.contains(s.id))
            .toList();
        targetSources = active.isNotEmpty ? active : allSources;
      }
      ...
      final targetSourceInfo = targetSources.firstWhereOrNull((s) {
      ...
      final futures = targetSources.map((info) async {
        try {
          final source = arg.type.usesAnimeSources
  ...
  Future<PaginatedResult<UnifiedMedia>> _fetchSourceSearchPage(
    ...
    // If viewing without query/genres/tags, browse trending/popular
